...
class UserNotificationService {
  ...
  Future<List<AppNotification>> getUserNotifications() async {
    ...
    if (_cachedNotifications != null &&
      ...
      Logger.info(
        '📋 Returning cached notifications (${_cachedNotifications!.length} items)',
      );
  ...
  Future<List<AppNotification>> _fetchNotifications() async {
    try {
      ...
      Logger.info(
        '📊 Generated ${notifications.length} notifications from user activity',
      );
      ...
      Logger.warning(
        '⚠️ Returning system notifications only due to API issues.',
      );
  ...
  Future<List<AppNotification>> _generateNotificationsFromUserActivity() async {
    ...
    try {
        ...
        _getUserRoleRequests()
            ...
            .then((roleRequests) {
              notifications.addAll(
                _createRoleChangeNotifications(roleRequests),
              );
            })
            .catchError((e) {
              Logger.debug(
                'Skipping role change notifications due to API issue: $e',
              );
            }),
      );
      ...
      Logger.warning(
        '⚠️ Error generating notifications from user activity: $e',
      );
    }
  ...
  Future<List<dynamic>> _getUserRegistrations() async {
    try {
      ...
      if (e.toString().contains('403')) {
        Logger.debug(
          'User does not have permission to access registrations (likely provider/admin user)',
        );
      } else {
  ...
  Future<List<dynamic>> _getUserRoleRequests() async {
    try {
      ...
      if (e.toString().contains('403')) {
        Logger.debug(
          'User does not have permission to access role change requests (likely provider/admin user)',
        );
      } else {
  ...
  List<AppNotification> _createTourNotifications(List<dynamic> registrations) {
    ...
    for (final registration in registrations) {
      try {
        final tourName =
            registration['custom_tour_id']?['tour_name'] ?? 'Your Tour';
        final status = registration['status'] ?? 'pending';
        final createdDate =
            DateTime.tryParse(registration['created_date'] ?? '') ??
            DateTime.now();
        ...
        switch (status) {
          case 'approved':
            ...
            body =
                'Your registration for "$tourName" has been approved. Get ready for an amazing experience!';
            type = 'tour_approved';
            break;
          case 'rejected':
            title = 'Tour Registration Update';
            body =
                'Your registration for "$tourName" requires attention. Please check the details.';
            type = 'tour_rejected';
            break;
          case 'pending':
            title = 'Registration Received';
            body =
                'We\'ve received your registration for "$tourName". We\'ll review it shortly.';
            type = 'tour_pending';
  ...
  List<AppNotification> _createRoleChangeNotifications(
    ...
    for (final request in roleRequests) {
      try {
        ...
        final createdDate =
            DateTime.tryParse(request['created_date'] ?? '') ?? DateTime.now();
        ...
        switch (status) {
          ...
          case 'rejected':
            ...
            body =
                'Your role change request has been reviewed. Please check the admin notes for details.';
            type = 'role_rejected';
            break;
          case 'pending':
            title = 'Role Change Request Submitted';
            body =
                'We\'ve received your role change request. Our team will review it shortly.';
            type = 'role_pending';
            isRead = true;
            break;
          default:
  ...
  List<AppNotification> _createSystemNotifications() {
    ...
    notifications.add(
      AppNotification(
        ...
        body:
            'Discover amazing tours and create unforgettable memories. Start exploring now!',
        type: 'system_welcome',
    ...
    notifications.add(
      AppNotification(
        ...
        body:
            'Tour providers can now send you real-time updates and messages about your tours!',
  ...
  Future<void> sendTestNotification() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Logger.warning(
          '⚠️ Test notification failed with status: ${response.statusCode}',
        );
      }
    } catch (e) {
  ...
  Future<bool> subscribeToPushNotifications(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Logger.warning(
          '⚠️ Push notification subscription failed with status: ${response.statusCode}',
        );
        return false;
      }
    } catch (e) {
